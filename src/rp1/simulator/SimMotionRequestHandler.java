...
public class SimMotionRequestHandler implements RsMotionRequestHandler {

   public SimMotionRequestHandler(SimClient client){
      this.client = client;
      verbosity   = client.session.getVerbosity();
   }

   public void processRequest(RsRequest t){
      process((RsMotionRequest)t);
   }
   ...
   public void process(RsMotionRequest request){
      ...
      task           = new SimStartMotionTask(client, request);
      task.setStartTime(simTime);
      if(verbosity)
         session.verbose("Adding start-motion request:\n  linearVelocity:      "+request.linearVelocity+
                                                     "\n  rotationalVelociity: "+request.rotationalVelocity+
